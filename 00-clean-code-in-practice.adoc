...
What is behind this gap? Is it that we don't believe in the benefits of clean code?
...
We target a well known Spring PetClinic app as an example of a source code
 that is on pair to with the code we do on our everyday basis.
...
Just like with the helmet, we trust ourselves with the code.
First of all, we do tests. We also use architecture frameworks Spring, Angular or something third.
So, if we are testing, if we're using MVC, the rest is cosmetic, right?
In addition, we don't have all the time in the world. And Clean Code requires time.

At least I speak about my personal perspective from the early days.
What changed for me is that I joined a project that is in production for a loooong time.
We deal with uber classes, measured in thousands of lines of code.
Do you think that the classes having several thousand lines is properly tested?
Do you think this class does something important?
...
There's something about the personal experience that makes you become a believer.
 My second read, was as reading Agatha Christie's novel.
...
Spring PetClinic is a good target as it is in a really good shape and familiar to millions. Its the only case of a demo application that has its own community, that I'm aware of.
In addition Spring as framework, just as Clean Code, is on a mission of enabling apps that are easy to change and evolve.

Following are the findings of Spring PetClinic review finding, sorted by the degree of drama:
